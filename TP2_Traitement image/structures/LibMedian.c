...
/* --- Balayage Video du voisinage 3x3 --- */
    ...
    for(i = 0; i < 3; i++)
    {
      ...
      /* Calcul des coordonnées du pixel suivant */
      i==3?POINT_X(pointv) = POINT_X(point) + i;
      if(j+i == 0) min = (short)PIXEL(image,pointv); //On prend le premier pixel étudié comme base
      else
      {
          if(PIXEL(image,pointv) >)
      }
